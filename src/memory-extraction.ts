...
If no significant memories are found, return an empty array: []`;
  ...
  try {
    ...
    if (result.status === 'error') {
      logger.error({ error: result.error, agentFolder }, 'Memory extraction failed');
      return [];
    }
    ...
    if (startIndex === -1 || endIndex === -1) {
      logger.warn({ agentFolder, response: responseText.slice(0, 200) }, 'No JSON array found in extraction response');
      return [];
    }
    ...
    const filteredMemories = memories.filter(m => {
      if (!validTypes.includes(m.type)) {
        logger.warn({ invalidType: m.type, content: m.content }, 'Invalid memory type, skipping');
        return false;
      }
      if (m.importance < 1 || m.importance > 10) {
        logger.warn({ invalidImportance: m.importance, content: m.content }, 'Invalid importance, defaulting to 5');
        m.importance = 5;
      }
    ...
    logger.info({ agentFolder, extractedCount: filteredMemories.length }, 'Memories extracted successfully');
...
Keep the summary under 200 words.`;
  ...
  try {
    ...
    if (result.status === 'error' || !result.result) {
      logger.error({ error: result.error, agentFolder }, 'Daily summary failed');
      return null;
    }
    ...
    if (startIndex === -1 || endIndex === -1) {
      logger.warn({ agentFolder, response: responseText.slice(0, 200) }, 'No JSON found in summary response');
      return null;
    }

    const jsonString = responseText.slice(startIndex, endIndex);
    const summaryData = JSON.parse(jsonString) as { summary: string; topics: string[] };
    ...
    logger.info({ agentFolder, date, topicCount: summaryData.topics.length }, 'Daily summary created');
    return summaryData;
  } catch (error) {
    logger.error({ error, agentFolder }, 'Exception during daily summary creation');
    return null;
  }
}
